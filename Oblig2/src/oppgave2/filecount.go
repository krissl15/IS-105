...
func filecount(s string) {
	...
	for i := 0; i < 5; i++ { //<- ignorer feilmeldingen i golanD
		fmt.Printf("%v %d %v %c %v %v %d\n", "rune:", ss[i].Key, "(", ss[i].Key, ")", "antall:", ss[i].Value) //output: hvilken rune og hvor mange ganger anvendt
	}
}
